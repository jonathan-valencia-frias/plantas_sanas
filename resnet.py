...
ase_model = ResNet50(weights='imagenet', include_top=False, input_shape=(Tamaño_imagen, Tamaño_imagen, 3))

# Congelar las capas de la base
for layer in ase_model.layers:
    layer.trainable = False

flatten_layer = layers.Flatten()
dense_layer_1 = layers.Dense(2048, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.01))
dense_layer_2 = layers.Dense(2048, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.01))
...
# Entrenar el modelo
model.fit(train_images, train_labels, epochs=50, validation_split=0.2, batch_size=32, callbacks=[EarlyStopping(monitor='val_accuracy', mode='max', patience=5, restore_best_weights=True)])
